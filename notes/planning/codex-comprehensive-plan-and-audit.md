...
Gaps:
- Subtree-level incremental patching is now only partially implemented (terminal + web root VBox/HBox child patching; desktop nested VBox/HBox subtree patching); cross-adapter subtree patching by element ID and deeper terminal/web patching are not implemented yet.
...
Status: root-level diff-aware updates complete; desktop nested VBox/HBox subtree patching is complete; terminal + web deeper subtree patching remains.
