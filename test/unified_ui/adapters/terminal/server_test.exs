defmodule UnifiedUi.Adapters.Terminal.ServerTest do
  use ExUnit.Case, async: true

  alias UnifiedUi.Adapters.Terminal.Server
  ...
  describe "lifecycle" do
    ...
    test "renders and stores renderer state" do
      iur_tree = %Widgets.Text{content: "Terminal server"}
      assert {:ok, server} = Server.start_link()

      assert {:ok, renderer_state} = Server.render(server, iur_tree)
      assert renderer_state.platform == :terminal
    ...
    test "update/3 updates existing renderer state" do
      ...
      assert updated_state.platform == :terminal
