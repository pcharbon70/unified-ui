defmodule UnifiedUi.Adapters.Desktop.Server do
  @moduledoc """
  GenServer lifecycle wrapper for the Desktop renderer.

  This server keeps renderer state between calls and exposes a runtime API for
  rendering, updating, and querying the current desktop render state.
  """
  ...
  @doc """
  Starts the desktop renderer server.
  ...
  * `:render_opts` - Default render options merged into every render/update call
  ...
  @doc """
  Stops the desktop renderer server.
  """
  ...
  def handle_call({:update, iur_tree, opts}, _from, state) do
    ...
    case Desktop.update(iur_tree, state.renderer_state, merged_opts) do
      {:ok, renderer_state} ->
  ...
  def terminate(_reason, %{renderer_state: renderer_state}) do
    _ = Desktop.destroy(renderer_state)
    :ok
  end
  ...
  defp do_render(iur_tree, opts, state) do
    ...
    case Desktop.render(iur_tree, merged_opts) do
      {:ok, renderer_state} ->
        {:reply, {:ok, renderer_state},
         %{state | renderer_state: renderer_state, iur: iur_tree, render_opts: merged_opts}}
